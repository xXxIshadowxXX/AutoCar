...
def detect_verplicht_links(img_np):
    ...
    for cnt in contours:
        ...
        if white_pixels > 500:
            return "Verplicht links rechthoek"
...
def board_handler(board_type):
    if board_type == "Verplicht links":
        ...
        stuurhoek_naar_robot_async(96)
    elif board_type == "Voorrangsbord":
        stuurhoek_naar_robot_async(93)
...
def snelle_loop():
    try:
        ...
        # Hier pas doorgaan met verwerking als we zeker een geldige foto hebben!
        # (lijnvolg, borddetectie etc...)
        ...
        if runai_enabled.get() and not runai_lock.locked():
            threading.Thread(target=runai_worker, daemon=True).start()
